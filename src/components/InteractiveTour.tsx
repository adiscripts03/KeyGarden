...
export const InteractiveTour: React.FC = () => {
  ...
  const steps = [
    {
      ...
      icon: <TreePine className="w-5 h-5 text-amber-400" />,
      content:
    ...
    },
    {
      ...
      icon: <Layers className="w-5 h-5 text-warm-300" />,
      content:
    ...
    },
    {
      ...
      icon: <ShieldCheck className="w-5 h-5 text-warm-300" />,
      content:
    ...
    },
    {
      ...
      badge: 'Paymaster & Gas Abstraction',
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 max-w-md w-full animate-slide-up">
      <div className="bg-surface-900 border border-surface-700 rounded-2xl p-5 shadow-2xl relative text-warm-100">
        {/* Close */}
        <button
          onClick={endTour}
        ...
        <div className="flex items-center gap-2 mb-2">
          <span className="text-[10px] uppercase font-semibold px-2 py-0.5 rounded-full bg-amber-400/10 text-amber-300 border border-amber-400/20">
            {current.badge}
          </span>
        ...
        <div className="flex items-center gap-2.5 mb-2">
          {current.icon}
          <h3 className="text-sm font-bold text-warm-50">{current.title}</h3>
        </div>

        {/* Content */}
        <p className="text-xs text-warm-300 leading-relaxed mb-4">
          {current.content}
        </p>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-surface-800">
          <button
            ...
            className={`p-2 rounded-lg text-xs font-medium flex items-center gap-1 transition-colors ${
              tourStep === 0 ? 'opacity-30 cursor-not-allowed text-warm-600' : 'text-warm-400 hover:text-white hover:bg-surface-800'
            }`}
          >
          ...
          <button
            onClick={handleNext}
            className="px-4 py-2 rounded-xl text-xs font-semibold text-surface-950 bg-amber-500 hover:bg-amber-400 flex items-center gap-1.5 shadow-sm transition-colors"
          >
